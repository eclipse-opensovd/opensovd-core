...
"""Tests for the opensovd-mcp --version CLI option."""
...
def test_cli_version(mcp):
    """--version outputs version info and exits.

    Ensures version, git hash, and date are correctly collected during build.
    """
    match = mcp.wait_for(VERSION_PATTERN, timeout_seconds=5.0)
    mcp.process.wait(timeout=5.0)
