...
def make_token(signing_key, algorithm, *, sub="testuser", roles=(), exp=None) -> str:
    """Create a signed JWT token."""
    if exp is None:
        exp = int(time.time()) + 3600
    payload = {"sub": sub, "roles": list(roles), "exp": exp, "iss": "OpenSOVD"}
...
def binary_args(request, auth_config):
...
def test_invalid_token_returns_401(client):
    ...
    response = client.get("/v1/components", headers={"Authorization": "Bearer not-a-valid-jwt"})
    assert response.status_code == 401


def test_expired_token_returns_401(client, auth_config):
    ...
    response = client.get("/v1/components", headers={"Authorization": f"Bearer {token}"})
    assert response.status_code == 401
...
def test_reader_get_allowed(client, auth_config):
...
def test_reader_put_denied(client, auth_config):
...
def test_admin_get_allowed(client, auth_config):
...
def test_admin_put_allowed(client, auth_config):
...
def test_no_roles_denied(client, auth_config):
